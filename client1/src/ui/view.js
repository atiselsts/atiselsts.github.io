...
RED.view = function() {
    ...
    function canvasMouseMove() {
        ...
        if (mouse_mode == RED.state.JOINING) {
            ...
            /* 
            var center_x = mousePos[0] - mousedown_node.x;
            var center_y = mousePos[0] - mousedown_node.y;
            var center_delta = Math.sqrt(center_x*center_x+center_y*center_y);
            // XXX: this might be quite frustrating I guess?
            if(center_delta > RED.options
                        drag_line.attr("class", "drag_line_hidden");
            console.log("center delta=" + center_delta); */
...
/*            if (d._def.inputs > 0) {
        ...
        nodes.each(function(d,i) {
            ...
            if (true) {
                ...
                // console.log("redraw node " + getValue(d._def.label, d));
    ...
    function redrawLinks(nodes)
    {
        ...
        links.each(function(d,i) {
            ...
            // show link properties; this code is disabled for now!
            /* if (d.dirty || redrawAllNodes) {
                d.dirty = false;

                // remove old
                $('.link_rect', this).remove();
                thisLink.selectAll('.link_property_label').text('')

                var props = getPropertiesForDisplay(d);
                if (props.length) {
                    thisLink.append("rect")
                    ...
                    for (var i in props) {
                        ...
                        thisLink.append("svg:text")
                            .attr('x',6).attr('y', yPos)
                            .attr('class', 'property_label small')
    ...
    /*
    $(' #dialog-visual-options-form ').fileUpload({
        success: function (data,textStatus,jqXHR) {showImportedBackground(data)},
        error: function (data,textStatus,err) {
            RED.notify("File upload failed or image type not recognized!", "error");
            showImportedBackground(null);
        }
    }); */
